...
<%	
	...
	vlist = pMgr.get_onetime_cars(c_data);
	if (vlist != null) {
...
function car_remove(car_num) {
	fetch("<%=cPath%>/api/place/onetime_remove.do?numb="+place_in_Frm.numb.value+"&car_num="+car_num, {
		method: "POST"
	}).then((res) => {
		if (res.status === 200) {
			location.reload();
		...
		<table width="900" cellpadding="3">
			<tr style="display: flex; flex-direction: column">
				...
				<td bgcolor=#dddddd height="21" align="center">
					아래 정보를 확인하고 회차 차량을 등록하거나 삭제 할수 있습니다.
				</td>
			</tr>
		</table>
		<form name="place_in_Frm" method="post" action="<%=cPath%>/api/place/onetime_add.do">
